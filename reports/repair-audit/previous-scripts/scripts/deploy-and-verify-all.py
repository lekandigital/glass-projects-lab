...
def main():
    ...
    for cand in candidates:
        ...
        # --- 3. VERCEL DEPLOY STEP ---
        # Determine VERCEL runnable root.
        # As discussed, we deploy the STATIC output folder directly for static/Vite apps
        # to avoid monorepo and bundler issues in the cloud, EXCEPT for nextjs.
        deploy_root = dir_to_serve
        if framework == 'nextjs':
            deploy_root = os.path.join(cwd, cand['relative_candidate_path'])
